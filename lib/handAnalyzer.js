...
HandAnalyzer.prototype.analyze = function(hand) {
    ...
    if (rank = this.isStraightFlush(straightCardRank, flushCardSuit)) {
        handType = this.handCount.intToRank(rank) === 'A' ? 9 : 8; // handle Royal Flush
    ...
    return {
        ...
        finalHand: finalHand,
        weight: this.getHandWeight(handType, finalHand)
    }
};
...
HandAnalyzer.prototype.getHandWeight = function(handType, hand) {
    var multiplier = 10 * Math.pow(10, handType);
